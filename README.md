...
| [stakes-gate](./stakes-gate) | A runnable gate that gives an AI room to make judgment calls without the final say: the model proposes a verdict, a deterministic wall it can't see or edit enforces the limits — outward or irreversible stakes always route to a human, and out-of-range confidence fails closed. Ships with the regression test for the bypass adversarial review actually found. |
| [premise-gate](./premise-gate) | A fourth gate class for AI-driven work: before anything runs, a deterministic lint flags "X asked for Y" claims that carry no source, so flawless output can't be built on a fabricated premise. Recall-biased on purpose, with a self-test that plants the original laundered claim. |
